...
# package -> vulns cache (debounced investigations re-run; packages don't change).
_cache: dict[tuple[str, str, str | None], list[dict]] = {}
...
async def query_osv(name: str, version: str | None = None, ecosystem: str = "npm") -> list[dict]:
    ...
    try:
        ...
        out = [{"id": v.get("id"), "summary": v.get("summary", ""), "aliases": v.get("aliases", [])}
               for v in vulns][:5]
        _cache[key] = out
        return out
